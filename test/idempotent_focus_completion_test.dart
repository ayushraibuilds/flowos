...
void main() {
  ...
  group('TASK-003: Transactional & Idempotent Focus Completion Tests', () {
    test(
      'FocusSessionService.completeSession is idempotent and creates exactly 1 XP ledger entry',
      () async {
        ...
        final ledgerEntries = await db.xpLedgerDao
            .select(db.xpLedgerEntries)
            .get();
        final sessionXpRows = ledgerEntries
            .where((e) => e.sourceEntityId == sessionId)
            .toList();
    ...
    test(
      'Calling stopSession on an already completed session does not downgrade or modify completion',
      () async {
        final sessionId = await service.startSession(
    ...
    test(
      'XpCalculator.awardSessionXP is idempotent by sourceEntityId',
      () async {
    ...
    test(
      'FocusTimerNotifier completeSession returns attached completionResult without duplicate calls',
      () async {
        ...
        final ledger = await db.xpLedgerDao.select(db.xpLedgerEntries).get();
        final sessionRows = ledger
            .where((e) => e.sourceEntityId == state.sessionId)
            .toList();
    ...
    test(
      'Flowtime manual completion is transactional and idempotent',
      () async {
        final sessionId = await service.startSession(
