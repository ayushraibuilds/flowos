...
void main() {
  ...
  group('TASK-002: Account-Scoped Local Storage & Sync Ownership Tests', () {
    test(
      'A -> logout -> B integration test: Account B cannot access or push Account A outbox',
      () async {
        // 1. User A active
        db.setActiveOwnerId('user_A');
        await db.tasksDao.insertTask(
          createTestTaskCompanion('task_A_1', 'User A Secret Task'),
        );
        ...
        await db.tasksDao.insertTask(
          createTestTaskCompanion('task_B_1', 'User B Task'),
        );
    ...
    test(
      'Offline queue / account-switch test: Outbox created for A is never uploaded as B',
      () async {
        // Offline writes under User A
        db.setActiveOwnerId('user_A');
        await db.tasksDao.insertTask(
          createTestTaskCompanion('offline_task_A', 'Offline Task A'),
        );

        // Offline writes under User B
        db.setActiveOwnerId('user_B');
        await db.tasksDao.insertTask(
          createTestTaskCompanion('offline_task_B', 'Offline Task B'),
        );
        ...
        expect(
          outboxA.map((e) => e.entityId),
          isNot(contains('offline_task_B')),
        );

        expect(outboxB.map((e) => e.entityId), contains('offline_task_B'));
        expect(
          outboxB.map((e) => e.entityId),
          isNot(contains('offline_task_A')),
        );
      },
    );
    ...
    test('Existing local-only data migration & claim test', () async {
      ...
      db.setActiveOwnerId('local');
      await db.tasksDao.insertTask(
        createTestTaskCompanion('local_task_1', 'First Local Task'),
      );
      ...
      final claimedOutbox = await db.syncOutboxDao.getUnsyncedForOwner(
        'first_user_123',
      );
      expect(claimedOutbox.length, equals(1));
      expect(claimedOutbox.first.entityId, equals('local_task_1'));

      final remainingLocal = await db.syncOutboxDao.getUnsyncedForOwner(
        'local',
      );
    ...
    test(
      'Interrupted migration & reopen test: Database reopens cleanly at v10',
      () async {
        db.setActiveOwnerId('user_X');
        await db.tasksDao.insertTask(
          createTestTaskCompanion('task_x_1', 'Task X'),
        );
    ...
    test('Account-scoped cursor watermarks test', () async {
      ...
      await prefs.setString(
        'flowos_sync_cursor_v2_${userA}_tasks_updated_at',
        '2026-07-27T10:00:00Z',
      );
      await prefs.setString(
        'flowos_sync_cursor_v2_${userA}_tasks_id',
        'task_A_cursor',
      );

      await prefs.setString(
        'flowos_sync_cursor_v2_${userB}_tasks_updated_at',
        '2026-07-27T12:00:00Z',
      );
      await prefs.setString(
        'flowos_sync_cursor_v2_${userB}_tasks_id',
        'task_B_cursor',
      );

      expect(
        prefs.getString('flowos_sync_cursor_v2_${userA}_tasks_updated_at'),
        equals('2026-07-27T10:00:00Z'),
      );
      expect(
        prefs.getString('flowos_sync_cursor_v2_${userB}_tasks_updated_at'),
        equals('2026-07-27T12:00:00Z'),
      );
    });
  });
}
