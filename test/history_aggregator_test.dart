...
void main() {
  group('HistoryAggregator V2 Tests', () {
    ...
    test(
      'getWeeklyAggregate averages only complete V2 days and excludes V1 days',
      () async {
        ...
        await db.dailyScoresDao.upsertScore(
        ...
        final aggregate = await HistoryAggregator.getWeeklyAggregate(
          db,
          monday,
        );
    ...
    test(
      'reclaimable time calculates delta strictly based on historic budget limits and distracting app flags',
      () async {
        final now = DateTime.now();
        final monday = DateTime(
          now.year,
