...
void main() {
  ...
  group('TASK-004: Focus Protection Mode Persistence & Lifecycle Tests', () {
    test('Three-mode start: Gentle writes nudge, Guardrail writes guard, Shield writes deep', () async {
      ...
      expect(policyNudge!.focus!.protectionMode, equals(ProtectionMode.nudge));
      ...
      expect(policyGuard!.focus!.protectionMode, equals(ProtectionMode.guard));
    ...
    test('Pause & Resume preserves start-time protectionMode (Shield remains deep)', () async {
    ...
    test('Rehydration restores persisted protectionMode from SharedPreferences', () async {
      ...
      await prefs.setString('flowos_active_session_type', SessionTypeColumn.deepWork.name);
      await prefs.setString('flowos_active_phase', FocusTimerPhase.running.name);
      await prefs.setInt('flowos_active_total_seconds', 90 * 60);
      await prefs.setString('flowos_active_protection_mode', ProtectionMode.deep.name);
    ...
    test('Legacy state without protectionMode defaults safely to ProtectionMode.guard', () async {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('flowos_active_session_id', sessionId);
      await prefs.setString('flowos_active_session_type', SessionTypeColumn.pomodoro.name);
      await prefs.setString('flowos_active_phase', FocusTimerPhase.running.name);
    ...
    test('Active scoped breaks survive pause/resume cycle on allowed policy', () async {
      final container = ProviderContainer(
        overrides: [
          databaseProvider.overrideWithValue(db),
          focusSessionServiceProvider.overrideWithValue(service),
      ...
      final policyWithBreak = SourcePolicy(
        ...
        scopedBreaks: [ScopedBreak(packageName: 'com.instagram.android', expiresAt: breakUntil, source: PolicySource.focus)],
      );
      ...
      expect(restoredPolicy.focus!.scopedBreaks.first.packageName, equals('com.instagram.android'));
    });
  });
}
