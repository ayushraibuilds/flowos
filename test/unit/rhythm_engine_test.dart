...
void main() {
  group('RhythmEngine Tests', () {
    test('Returns null if sessions count is less than threshold', () {
      final sessions = List<FocusSession>.generate(
        4,
        (i) => FocusSession(
          id: 'session_$i',
    ...
    test('Identifies the peak 2-hour window and preferred weekday correctly', () {
      ...
      final r = rec!;
      expect(r.windowStartHour, 8);
      expect(r.windowEndHour, 10);
      expect(r.preferredWeekday, 1); // Monday
      expect(r.headline, contains('8 AM - 10 AM'));
      expect(r.actionLabel, contains('Monday'));
      expect(r.evidence, contains('6 sessions'));
      expect(r.evidence, contains('Avg grade A'));
    });
  });
}
