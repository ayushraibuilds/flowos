...
void main() {
  ...
  group('Onboarding Welcome Screen Tests', () {
    testWidgets(
      'Renders sprouting plant and trust copy, and handles skip/continue buttons',
      (tester) async {
        ...
        expect(
          find.textContaining('Your device activity stays on this device'),
          findsOneWidget,
        );
  ...
  group('Onboarding Rhythm Screen Tests', () {
    testWidgets(
      'Goals multiselect validation and focus minutes segmented choice',
      (tester) async {
    ...
    Widget createConnectScreenWidget() {
      return ProviderScope(
        overrides: [
          ...
          deviceAttentionPlatformProvider.overrideWithValue(
            MockDeviceAttentionPlatform(),
          ),
        ...
        child: MaterialApp(
          ...
          home: Scaffold(body: OnboardingConnectScreen(onComplete: () {})),
        ),
      );
    }

    testWidgets('Renders app selection card and device integrations list', (
      tester,
    ) async {
      ...
      expect(
        find.textContaining('Accessibility Blocker (Optional)'),
        findsOneWidget,
      );
