...
void main() {
  ...
  group('M0 Audit Tests', () {
    test(
      'DailyScoreCalculator reweights properly when attention data is not connected',
      () {
    ...
    test(
      'AttentionDataRepository.syncUsage ignores empty package rows but marks day complete',
      () async {
        ...
        platform.dailyUsage = [
          ...
          },
          {
            'date': nowStr,
            'packageName':
                '', // dummy placeholder for zero distraction usage day
            'label': '',
            'minutes': 0,
          },
        ];
        ...
        final metric = await db.deviceDayMetricsDao.getForDay(
          todayDate,
          'android',
        );
    ...
    test(
      'Drift Schema V6 migration preserves daily reports table integrity',
      () async {
        final db = AppDatabase.forTesting(NativeDatabase.memory());
