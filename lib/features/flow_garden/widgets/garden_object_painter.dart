...
class GardenObjectPainter extends CustomPainter {
  ...
  void _paintTree(Canvas canvas, Size size, Offset center, double radius) {
    ...
    if (emoji == '🌲') {
      ...
      ).createShader(Rect.fromLTRB(center.dx - radius, topY, center.dx + radius, bottomY));

      canvas.drawPath(pinePath, leafPaint);

    } else if (emoji == '🌳') {
      // Fluffy rounded Oak Tree
      final foliageRect = Rect.fromCircle(center: Offset(center.dx, center.dy - radius * 0.15), radius: radius * 0.65);
      leafPaint.shader = RadialGradient(
        colors: [const Color(0xFF4CAF50), const Color(0xFF2E7D32)],
        center: const Alignment(-0.2, -0.2),
      ).createShader(foliageRect);

      canvas.drawCircle(Offset(center.dx, center.dy - radius * 0.2), radius * 0.5, leafPaint);
      canvas.drawCircle(Offset(center.dx - radius * 0.25, center.dy - radius * 0.1), radius * 0.38, leafPaint);
      canvas.drawCircle(Offset(center.dx + radius * 0.28, center.dy - radius * 0.12), radius * 0.38, leafPaint);
      canvas.drawCircle(Offset(center.dx, center.dy - radius * 0.42), radius * 0.38, leafPaint);
      ...
      for (int angleDeg in [-40, -15, 10, 170, 195, 220]) {
        ...
        final controlY = topCenter.dy - radius * 0.3 + math.sin(angle) * radius * 0.2;
  ...
  void _paintFlower(Canvas canvas, Size size, Offset center, double radius) {
    ...
    final leafLeft = Path()
      ...
      ..quadraticBezierTo(center.dx - radius * 0.3, center.dy + radius * 0.35, center.dx, center.dy + radius * 0.2)
      ..close();
    ...
    if (emoji == '🌸') {
      ...
      // Yellow Center
      canvas.drawCircle(center, radius * 0.22, Paint()..color = const Color(0xFFFFD54F));
      ...
      for (int i = 0; i < 12; i++) {
        ...
        final rect = Rect.fromCenter(center: Offset(0, -radius * 0.35), width: petalWidth, height: petalHeight);
        canvas.drawOval(rect, petalPaint);
        canvas.restore();
      }
      // Dark Center
      canvas.drawCircle(center, radius * 0.32, Paint()..color = const Color(0xFF4E342E));

    } else if (emoji == '🌷') {
      // Tulip (Red Cup Shape)
      final tulipRect = Rect.fromCenter(center: center, width: radius * 0.8, height: radius * 0.9);
      ...
      final path = Path()
        ...
        ..quadraticBezierTo(center.dx - radius * 0.45, center.dy + radius * 0.3, center.dx, center.dy + radius * 0.42)
        ..quadraticBezierTo(center.dx + radius * 0.45, center.dy + radius * 0.3, center.dx + radius * 0.32, center.dy - radius * 0.3)
        ..quadraticBezierTo(center.dx, center.dy - radius * 0.05, center.dx - radius * 0.32, center.dy - radius * 0.3)
        ..close();
      ...
      canvas.drawPath(
        Path()
          ...
          ..quadraticBezierTo(center.dx - radius * 0.25, center.dy, center.dx, center.dy + radius * 0.4)
          ..quadraticBezierTo(center.dx + radius * 0.25, center.dy, center.dx + radius * 0.1, center.dy - radius * 0.38)
          ..quadraticBezierTo(center.dx, center.dy - radius * 0.2, center.dx - radius * 0.1, center.dy - radius * 0.38)
          ..close(),
      ...
      // Orange Center
      canvas.drawCircle(center, radius * 0.25, Paint()..color = const Color(0xFFFFB300));
    }
  }
  ...
  void _paintButterfly(Canvas canvas, Size size, Offset center, double radius) {
    ...
    final leftWing = Path()
      ...
      ..cubicTo(center.dx - radius * 0.8, center.dy - radius * 0.8, center.dx - radius * 0.9, center.dy + radius * 0.1, center.dx - radius * 0.2, center.dy + radius * 0.2)
      ..cubicTo(center.dx - radius * 0.7, center.dy + radius * 0.7, center.dx - radius * 0.3, center.dy + radius * 0.8, center.dx, center.dy + radius * 0.1)
      ..close();
    ...
    canvas.drawOval(
      Rect.fromCenter(center: center, width: radius * 0.14, height: radius * 0.8),
      bodyPaint,
    );
    ...
    final lAnt = Path()
      ...
      ..quadraticBezierTo(center.dx - radius * 0.15, center.dy - radius * 0.5, center.dx - radius * 0.22, center.dy - radius * 0.55);
    final rAnt = Path()
      ..moveTo(center.dx + radius * 0.03, center.dy - radius * 0.35)
      ..quadraticBezierTo(center.dx + radius * 0.15, center.dy - radius * 0.5, center.dx + radius * 0.22, center.dy - radius * 0.55);
  ...
  void _paintDefaultSeedling(Canvas canvas, Size size, Offset center, double radius) {
    ...
    final stemPath = Path()
      ...
      ..quadraticBezierTo(center.dx - radius * 0.1, center.dy + radius * 0.1, center.dx + radius * 0.05, center.dy - radius * 0.2);
    ...
    final leafLeft = Path()
      ...
      ..quadraticBezierTo(center.dx - radius * 0.35, center.dy - radius * 0.3, center.dx - radius * 0.15, center.dy - radius * 0.05)
      ..close();
