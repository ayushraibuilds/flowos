...
final insightPeriodProvider = StateProvider<InsightPeriod>(
  (ref) => InsightPeriod.today,
);
...
final insightFocusTimelineProvider = FutureProvider<List<FocusSession>>((
  ref,
) async {
  final period = ref.watch(insightPeriodProvider);
  final db = ref.watch(databaseProvider);
...
final insightAppImpactProvider =
    FutureProvider<
      List<({String label, String packageName, int minutes, int budget})>
    >((ref) async {
      final period = ref.watch(insightPeriodProvider);
      final db = ref.watch(databaseProvider);
...
final insightInterruptionProvider =
    FutureProvider<
      ({
        bool isAvailable,
    ...
    >((ref) async {
      ...
      if (!hasConsent) {
        return (
          isAvailable: false,
          notificationCounts: <({String appName, String appRef, int count})>[],
          totalUnlocks: 0,
        );
      }
      ...
      for (final n in notifs) {
        ...
        if (current == null) {
          ...
          groupedNotifs[n.appRef] = (
            label: n.displayName,
            count: current.count + n.count,
          );
        }
      }

      final sortedNotifs =
          groupedNotifs.entries
              .map(
                (e) => (
      ...
      final totalUnlocks = dayMetrics.fold<int>(
        0,
        (sum, m) => sum + (m.unlockCount ?? 0),
      );
...
final insightCalendarHeatmapProvider = FutureProvider<List<DailyMetric>>((
  ref,
) async {
  final db = ref.watch(databaseProvider);
...
final insightsEnergyForecastProvider =
    FutureProvider<
      ({
        bool hasEnoughData,
        int totalCount,
    ...
    >((ref) async {
      ...
      if (totalCount < 7) {
        return (
          ...
          peaks: <({String end, String label, double level, String start})>[],
        );
      }
      ...
      final List<({String start, String end, String label, double level})>
      peaksList = [];
...
final insightsCompletionFunnelProvider =
    FutureProvider<({int created, int started, int completed})>((ref) async {
      final db = ref.watch(databaseProvider);
      ...
      return (created: created, started: started, completed: completed);
    });

/// Aggregates focus protection unlock attempts statistics for the selected period
final insightUnlockAttemptsProvider =
    FutureProvider<
      ({
        bool hasData,
    ...
    >((ref) async {
      ...
      final attempts =
          await (db.select(db.unlockAttempts)..where(
                (t) =>
