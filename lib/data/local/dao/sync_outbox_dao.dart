...
class SyncOutboxDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<SyncOutboxData>> getUnsynced() =>
      (select(syncOutbox)
  ...
  Future<void> claimLocalOutbox(String targetOwnerId) async {
    await (update(syncOutbox)..where((s) => s.ownerId.equals('local'))).write(
      SyncOutboxCompanion(ownerId: Value(targetOwnerId)),
    );
  }

  Future<void> insertOp(SyncOutboxCompanion entry) =>
      into(syncOutbox).insert(entry);

  Future<void> markSynced(String id) =>
      (update(syncOutbox)..where((s) => s.id.equals(id))).write(
