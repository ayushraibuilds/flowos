...
class DailyScoresDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<DailyScore?> getForDay(DateTime date) {
    ...
    return (select(
      dailyScores,
    )..where((s) => s.day.equals(startOfDay))).getSingleOrNull();
  }
  ...
  Stream<DailyScore?> watchForDay(DateTime date) {
    ...
    return (select(
      dailyScores,
    )..where((s) => s.day.equals(startOfDay))).watchSingleOrNull();
  }
  ...
  Future<void> deleteForDay(DateTime date) {
    final startOfDay = DateTime(date.year, date.month, date.day);
    return (delete(dailyScores)..where((s) => s.day.equals(startOfDay))).go();
  }
